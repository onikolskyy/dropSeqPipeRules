...
for ref in refs:
    reads_for_ref["ref"] = [read for read in indexed_bam.fetch(ref)]
...
refs_df = pd.DataFrame({"ref":refs})

for ref, group in refs_df.groupby("ref"):
    reads_for_ref[ref] = [reads[i] for i in group.index.to_list()]
