...
def tag_read_with_functional_data(bam_read, gene_interval_tree):
    ...
    bam_read.set_tag(Tags.tags_dict["GENE_FUNCTION_TAG"],  ", ".join(lf for lf_list in lf_lists for lf in lf_list))
    bam_read.set_tag(Tags.tags_dict["GENE_NAME_TAG"], ", ".join(gene_ids))
    bam_read.set_tag(Tags.tags_dict["GENE_STRAND_TAG"],   ", ".join(["-" if gene_interval_tree[gene_id].is_negative_strand() else "+" for gene_id in gene_ids]))
